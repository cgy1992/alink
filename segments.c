...
PSEG addSeg(PSEG s,PSEG c)
{
	...
	s->contentList=(PCONTENT)checkRealloc(s->contentList, (s->contentCount+1)*sizeof(CONTENT));
...
PSEG addFixedData(PSEG s,PDATABLOCK c)
{
	...
	/* add datablock to content list for parent */
	s->contentList=(PCONTENT)checkRealloc(s->contentList, (++s->contentCount)*sizeof(CONTENT));
